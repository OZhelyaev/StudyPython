...
# обучение модели
def model_training(df: DataFrame, model: LogisticRegression):
    # 1.10. Разделим датасет на обучающую и валидационные части (train и val) при помощи функции train_test_split
    x = df.drop(columns=['isAlive']).values  # только значения без данных о выживании
    y = df['isAlive'].values  # выделим целевые значения
...
# обучение модели
def model_test(df: DataFrame, model: LogisticRegression):
    # 1.10. Разделим датасет на обучающую и валидационные части (train и val) при помощи функции train_test_split
   ## x = df.drop(columns=['isAlive']).values  # только значения без данных о выживании
   ## y = df['isAlive'].values  # выделим целевые значения
    ...
    print(' test model с sklearn pred_train: ', pred_test)
    return pd.Series(pred_test)

    #  не можем посчитать метрики т.к. нет резкльтата
    # mse_test = mean_squared_error(y, pred_test)
    # rmse_test = sqrt(mse_test)
    # r2_test = r2_score(y, pred_test)
    # mae_test = mean_absolute_error(y, pred_test)
...
def save_result(df):
    ...
    submission['isAlive'] = df
    submission.to_csv('../data/new_submission.csv', index=False)

# подготовка данных модели
def df_preprocessing(data: DataFrame):
    # ------------------------  popularity  ---------------------------------------------------------------
    ...
    # ------------------------  Age, dateOfBirth  ---------------------------------------------------------------
    # Признак Age внем очень много пропущенных значений. Для того чтобы использовать в модели
    ...
    print('------ age < 0 ------------', data.loc[data['age'] < 0])
    if len(data.loc[data['age'] < 0]) > 0:
    ...
    # аналогичным образом преобразовываем признак dateOfBirth. У одних и тех же персонажей не
    ...
    ##plt.figure(figsize = (10, 5))
    ## plt.show()
    #  'dateOfBirth_no_data' - удилил т.к. совпадает с age_no_data
    print(data[['age', 'age_value', 'age_no_data', 'dateOfBirth']].head(10))
    print('-' * 30)
    ...
    cultures_grouped = {
        ...
        'Essos Nations': ['astapor', 'astapori', 'braavosi', 'braavos', 'tyroshi', 'lysene', 'lyseni',
                          ...
                          'ghiscaricari'],
        'Other Nations': ['ibbenese', 'westeros', 'free folk', 'wildling', 'wildlings', 'naathi', 'stormlander',
                          'stormlands']}
